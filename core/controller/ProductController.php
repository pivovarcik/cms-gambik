...
class ProductController extends PageVersionBase
{
	...
	{
		parent::__construct("Products", "ProductVersion");
	...
	public function setAttributeProduct($product_id){
		...
		//Application_Form_ProductAtributeEdit_action
		...
		if(self::$getRequest->isPost() &&
			false !== self::$getRequest->getPost('Application_Form_ProductAtributeEdit_action', false))
		{
			$data = array();
	...
	public function getProduct($product_id){
		...
		$attributes = new models_Attributes();
		$this->attributes = $attributes->get_attribute_value_association2($product_id);
	...
	public function productListZboziFeedLite(IListArgs $params)
	{
...
';
			...
			if (!empty($list[$i]->serial_cat_title)) {
				...
				if (strpos($nazev_vyrobce,"|")) {
					$nazev_vyrobce = substr($nazev_vyrobce,strpos($nazev_vyrobce,"|")+1,strlen($nazev_vyrobce));
				}
...
';
			...
			//$delivery_date = ($list[$i]->hodiny_dostupnost > 0) ? ($list[$i]->hodiny_dostupnost/24) : 0;


			/*$delivery_date="";

			if (trim(strtolower($list[$i]->dostupnost))=='běžně skladem' || trim(strtolower($list[$i]->dostupnost))=='skladem') {
				$delivery_date=0;
			}


			if (trim(strtolower($list[$i]->dostupnost))=='na objednávku') {
				$delivery_date=31;
			}

			if (trim(strtolower($list[$i]->dostupnost))=='skladem do týdne') {
				$delivery_date=7;
			}

			if (trim(strtolower($list[$i]->dostupnost))=='skladem do 3 dnů') {
				$delivery_date=3;
			}
			...
			if ($list[$i]->dostupnost=='Neznámá-nutno ověřit u výrobce' || strpos($list[$i]->dostupnost,"nutno")) {
				$delivery_date=8;
			}
...
';
			//URL_HOME2 .
			$url_img = (!empty($list[$i]->file)) ?  URL_HOME2 . $imageController->getZmensitOriginal($list[$i]->file,180,160) : "";
			//$url_img = "";
			//$img  = $xml->add_node($item, 'IMGURL');
			//$xml->add_cdata($img, str_replace("cz//","cz/",$url_img));
...
';
				...
				$res.='<PRICE_VAT>' . $list[$i]->prodcena . '</PRICE_VAT>
';
			}
...
/*
		$child = self::$getRequest->getQuery('child', '');
		...
		$skupina = self::$getRequest->getQuery('cat', '');
		if (isset($params['skupina']) && is_numeric($params['skupina'])) {
			$skupina =  (int) $params['skupina'];
		}
		$params2['skupina'] =  $skupina;



		// Nejnižší cena
		$lowestPrice = self::$getRequest->getQuery('lowestPrice', '');
		if (!empty($lowestPrice) && $lowestPrice*1 == $lowestPrice) {
			$params2['lowestPrice'] = (int) $lowestPrice;
		}

		if (isset($params['lowestPrice']) && is_numeric($params['lowestPrice'])) {
			$params2['lowestPrice'] = (int) $params['lowestPrice'];
		}

		// Nejvyšší cena
		$highestPrice = self::$getRequest->getQuery('highestPrice', '');
		if (!empty($highestPrice) && $highestPrice*1 == $highestPrice) {
			$params2['highestPrice'] = (int) $highestPrice;
		}
		if (isset($params['highestPrice']) && is_numeric($params['highestPrice'])) {
			$highestPrice = (int) $params['highestPrice'];
		}
...
//print $lowestPrice;
		$vyrobce = self::$getRequest->getQuery('vyr', '');
		if (isset($params['vyrobce']) && is_numeric($params['vyrobce'])) {
			$vyrobce = (int) $params['vyrobce'];
		}
		$params2['vyrobce'] = $vyrobce;
		$attributes = array();
		if (self::$getRequest->getQuery('sirka',0) !== 0) {
			$attributes[] = self::$getRequest->getQuery('sirka',0);
		}
		if (self::$getRequest->getQuery('profil',0) !== 0) {
			$attributes[]  = self::$getRequest->getQuery('profil',0);
		}

		if (self::$getRequest->getQuery('prumer',0) <> 0) {
			$attributes[]  = self::$getRequest->getQuery('prumer',0);
		}

		if (self::$getRequest->getQuery('obdobi',0) <> 0) {
			$attributes[]  = self::$getRequest->getQuery('obdobi',0);
		}
		...
		if (isset($params['attributes']) && is_array($params['attributes'])) {
			$params2['attributes'] = $params['attributes'];
		}


		if (isset($params['not_product_id']) && is_int($params['not_product_id'])) {
			$params2['not_product_id'] = $params['not_product_id'];
		}


		$category = self::$getRequest->getQuery('tree', 0)*1;
		if (isset($params['umisteni']) && is_numeric($params['umisteni'])) {
			$category = (int) $params['umisteni'];
		}
		$params2['kategorie'] = $category;

		$aktivni = self::$getRequest->getQuery('akt', '');
		if (isset($params['aktivni']) && is_numeric($params['aktivni'])) {
			$aktivni = $params['aktivni'];
		}
		$params2['aktivni'] = $aktivni;
		...
		$querys[] = array('title'=>'Cena','url'=>'price','sql'=>'v.prodcena');
		$querys[] = array('title'=>'Množ.','url'=>'qty','sql'=>'p.qty');

		$orderFromQuery = 'p.cislo DESC';
		if (isset($params['order_default']) && !empty($params['order_default']))
		{
			$orderFromQuery = $params['order_default'];
		}

		$orderFromQuery = $this->orderFromQuery($querys, $orderFromQuery);
...
*/
		...
		for($i=0;$i<count($l);$i++) {
			...
			$l[$i]->pagetitle = !empty($l[$i]->pagetitle) ? $l[$i]->pagetitle : $l[$i]->title;
			//$detail->pagekeywords = !empty($detail->pagekeywords) ? $detail->pagekeywords : $detail->titulek;
			$l[$i]->pagedescription = !empty($l[$i]->pagedescription) ? $l[$i]->pagedescription : $l[$i]->description;

			if (!empty($params->lang)) {
				$l[$i]->link = URL_HOME2 .$params->lang . "/" . get_categorytourl($l[$i]->serial_cat_url) . '/'. $l[$i]->page_id . '-' . strToUrl($l[$i]->title) . '.html';
			} else 			{
							$l[$i]->link = URL_HOME . get_categorytourl($l[$i]->serial_cat_url) . '/'. $l[$i]->page_id . '-' . strToUrl($l[$i]->title) . '.html';
			}
	...
	public function akcePoUlozeni()
	{
		...
		$formAtributy = new Application_Form_ProductEdit();
...
*/
	...
	private function copyProduct($product_id)
	{
		$model = new models_Products();
		...
		$data["skupina_id"] = $obj->skupina_id;
		...
		$attributesList = $attributes->get_attribute_value_association2($product_id);
	...
	public function copyAction()
	{
		...
		//	print_r($tenzin);
			if (is_array($tenzin)) {
				...
				$postdata = $form->getValues();
				$product_id = $postdata["id"]; //$_POST['Application_Form_ProductEdit_id'];
			}
