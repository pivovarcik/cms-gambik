...
class BasketController extends G_Controller_Action
{
	...
	public function getCenaDopravyValue()
	{
		...
		return $this->cena_dopravy_value . ' ' . MENA;;
	}
	...
	//	print_r($list);
		if (count($list)>0) {
			...
			$this->cena_dopravy_value = $list[0]->price_value * $jednotek;
		} else {
			//print $model->getLastQuery();
		}
		return $list;
...
/*
	...
	//	print "řádků:" . count($l);
		...
		for ($i=0;$i < count($l);$i++)
		{
			...
			// částka bez DPH
			$this->total_price += $zaklad * $l[$i]->mnozstvi;

			// částka s DPH
		//	$this->total_price_sdph += ($l[$i]->cena_sdph * $l[$i]->mnozstvi);

			$this->total_price_sdph += $l[$i]->cena_sdph_po_sleve * $l[$i]->mnozstvi;
...
*/
	...
	//	$g = new Zakl();
		...
		for ($i=0;$i < count($l);$i++)
		{
			...
			if (!empty($l[$i]->file)) {
				...
				$PreviewUrl = '<img alt="'.$l[$i]->title.'" src="' . $imageController->get_thumb($l[$i]->file,$thumb_width,$thumb_height,null,false,true) . '" class="imgobal" />';
			} else {
				$PreviewUrl = '';
			}
			...
			$l[$i]->mnozstvi_edit = $elemQty->render();
				...
				if ($eshopSettings->get("PRICE_TAX") == "0") {
					...
					$l[$i]->cenamj = numberFormat($l[$i]->price, $this->desetinna_mista) . ' ' . MENA;
				} else {
	...
	/*		if ($product_id) {
			...
			if ($isProduct == false) {
				// neexistuje, přidávám
				$model->addProduct($product_id, $basket_id, $qty, $productDetail->cena_bezdph,null, $varianty_id);
				if ($tandem_id) {
					$model->addProduct($tandem_id, $basket_id, $qty, null, $product_id, $varianty_id);
				}
				if ($tandem_id2) {
