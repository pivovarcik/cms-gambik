...
class MailingController extends G_Controller_Action
{
	...
	//	if (!empty($settings->get("EMAIL_SMTP_CERT"))) {
...
/*
	...
	{
		if($this->getRequest->isPost() && "ins_mail" === $this->getRequest->getPost('Application_Form_MailCreate_action', false))
		{
			// načtu Objekt formu
	...
	// zapíše email do DB
	private function createEmail($komu,$subject,$body,$visit)
	{
		$model = new models_Mailing();
	...
	public function odeslatEmail($komu,$predmet,$zprava,$priloha='',$prilohaName='',$skryta_kopie=false)
	{
		if (isEmail($komu)) {
			$visit = strtolower(substr(md5(rand()),0,50));
