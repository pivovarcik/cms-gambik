...
class SettingsTabs extends G_Tabs {
	...
	//	$contentMain .= '<p class="desc">Nastavení defaultní velikosti miniatur. Např. 100x100, 150x150.</p><br />';
		...
		$contentMain .= '<p class="desc">Url adresa ikonky (např. loga webu). Doporučujeme 16x16 a typ ICO.</p>';
	...
	protected function JazykyTabs()
	{
		...
		for ($i=0;$i<count($list);$i++)
		{
			...
			$button = ' <button type="submit"'.$disabled.' name="lang_active_'.$list[$i]->code.'" class="btn btn-primary">Deaktivovat</button>';
			if ($list[$i]->active == 0) {
				$button = ' <button type="submit" name="lang_active_'.$list[$i]->code.'" class="btn btn-primary">Aktivovat</button>';
			}
