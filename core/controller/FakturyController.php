...
class FakturaTabs extends G_Tabs {
	...
	protected function MainTab()
	{
		...
		if ($form->getElement("id")) {
			...
			$contentMain .=' <a target="_blank" href="' . URL_HOME . 'faktura_pdf.php?id=' . $form->getElement("id")->getValue(). '">Tisk PDF</a>';
		}
		$contentMain .= '<p class="desc">Číslo faktury.</p><br />';
		...
		$contentMain .= '<p class="desc">Datum vystavení faktury.</p><br />';
	...
	protected function FakturacniTab()
	{
		...
		$contentFak .= '<p class="desc"></p><br />';
		$contentFak .= $form->getElement("shipping_ico")->render() . ' <a href="#" class="ares">Ares</a>';
		$contentFak .= '<p class="desc"></p><br />';
		$contentFak .= $form->getElement("shipping_dic")->render();
		$contentFak .= '<p class="desc"></p><br />';
		$contentFak .= $form->getElement("shipping_phone")->render();
		$contentFak .= '<p class="desc"></p><br />';
		$contentFak .= $form->getElement("shipping_email")->render() . ' <a href="mailto:' . $form->getElement("shipping_email")->getValue() . '">odeslat email</a>
					<p class="desc"></p>';
...
if (!empty($doklad->nazev_dopravy)){
	...
	//	$eshopController = new EshopController();
		...
		$html = $this->createHtml($doklad_id);
