...
class models_Products extends models_Pages{
...
//print_r($params);
		...
		if (isset($params->fulltext) && !empty($params->fulltext)) {
			...
			v.perex like '%" . $params->fulltext . "%' or vb.name like '%" . $params->fulltext . "%') ");
		}
		...
		if (isset($params->skupina) && is_array($params->skupina) && count($params->skupina) > 0) {
			...
			if ( implode(",", $vyrobceA) == 0) {
				$vyrobceA = $params->skupina;
				//	$vyrobceA = array_flip($params->skupina);
				//	print_r($vyrobceA);
			}
			$this->addWhere("p.skupina_id in (". implode(",", $vyrobceA) . ")");
		}
	...
	/*	if (isset($params->fulltext) && !empty($params->fulltext)) {
		...
		d.name as nazev_dostupnost,
		d.hodiny as hodiny_dostupnost,
		v.id as version_id,vz.name as nazev_zaruka
		");
		}
...
//print_r($list);
		...
		if(defined("USER_ID"))
		{
			...
			if (isset($catalog) && $catalog->cenik_id > 0) {
	...
	private function vypocetCenyNaRadku($radek)
	{
		...
		if (isset($radek->pricelist) && count($radek->pricelist) > 0) {
			...
			if ($cenik->cenik_cena > 0) {
				$radek->prodcena = $radek->cenik_cena;
			}

			$radek->bezna_cena = $radek->prodcena;
		}
		$castka_dph = ($radek->value_dph>0) ? $radek->value_dph/100 : $radek->value_dph * 1;
		...
		$radek->castka_dph = $castka_dph * $zaklad;
...
*/
  ...
  max(case when dph.value <> 0 then v.prodcena*dph.value/100+v.prodcena else v.prodcena end)  as cenasdph_do ";
	...
	public function getSkupinyProductList(IListArgs $args = null)
	{
		...
		$args->orderBy	 = 't2.name ASC';
		$args->groupBy = 't2.id';
		$args->select = "t2.id,t2.name,count(p.id) as pocet,min(v.prodcena) as cena_od,max(v.prodcena) as cena_do";
		$list = $this->getList($args);
		return $list;
	}
}