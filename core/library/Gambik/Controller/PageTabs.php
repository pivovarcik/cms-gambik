	...
	protected function SeoTabs()
	{
		...
		foreach ($languageList as $key => $val)
		{
			if ($form->getElement("url_$val->code")) {
				...
				$style = ($first) ? "display:block;" : "display:none;";$first = false;
				$contentSeo .= '<div class="lang lang_' .$val->code. '" style="' .$style. '">' . $form->getElement("url_$val->code")->render() . '</div>';
			}
		}
		$contentSeo .= '<p class="desc"></p><br />';

		$contentSeo .= '<dl><dd><label for="url_cs">Odkaz:</label>
<a target="_blank" href="' . $form->page->link . '">' . $form->page->link . '</a></dd></dl>';
