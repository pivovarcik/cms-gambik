...
define('T_STREDISKA', DB_PREFIX . 'ucstr');
...
define("T_SHOP_PLATBY","mm_shop_platby");
...
class Sql
{
	...
	public static function spojit()
	{
		...
		self::$pripojeni = 	mysql_connect(DB_HOST, DB_USER, DB_PASS);

		if(!mysql_select_db(DB_NAME, self::$pripojeni)) { echo "<h1>Probíhá údržba systému</h1><p>Omlouváme se - Server je dočasně nedostupný. Na serveru právě probíhá údržba systému.<br />Přístupný bude v co nejkratší době.<br />Děkujeme za pochopení.</p><p><strong>Administrator</strong></p>"; exit(); }
		if (function_exists('mysql_set_charset')) {
			mysql_set_charset('utf8',self::$pripojeni);
		} else {
	...
	public static function escape($string)
	{
		if (self::$escape_string) {
			$string = mysql_real_escape_string($string,self::$pripojeni);
		}
...
/*
   if ($this->protokol) {
   $this->insert_protokol();
   }
*/
		// Perform the query via std mysql_query function..
		self::$result = mysql_query($query, self::$pripojeni);
		self::$num_queries++;
		...
		foreach ( $query_type as $word )
		{
			...
			{
				self::$rows_affected = mysql_affected_rows();

				// This gets the insert ID
				if ( $word == "insert" || $word == "replace" )
				{
					self::$insert_id = mysql_insert_id(self::$pripojeni);
	...
	{
		//mysql_autocommit(self::$pripojeni, $auto);
	...
	public static function konec_transakce($all_query_ok)
	{
		...
		//mysql_rollback(self::$pripojeni);
		//$all_query_ok ? mysql_commit(self::$pripojeni) : mysql_rollback(self::$pripojeni);
	...
	function print_error($str = "")
	{
		...
		if ( !$str ) $str = mysql_error(self::$pripojeni);
