...
$GHtml->setServerJs(URL_HOME_SITE . "js/fileupload/jquery.fileupload.js");
...
if (isset($_GET['item']))
{
	require_once PATH_ADMIN_PRG . 'clanek.php';
	return;
} elseif (isset($_GET['cat']) && !is_array($_GET['cat']))
{
	if (file_exists(PATH_ADMIN_PRG . $_GET['cat'] . '.php')){

		require_once  PATH_ADMIN_PRG . $_GET['cat'] . '.php';
	} else
	{

		require_once PATH_ADMIN_PRG . 'clanky.php';
	}
	return;
}
...
if (isset($_GET['url']) && !empty($_GET['url']))
{
	...
	//print $posledni;
	if (file_exists(PATH_ADMIN_PRG . $posledni.'.php')){
		require_once PATH_ADMIN_PRG . $posledni. '.php';
	}
	elseif (file_exists(PATH_ROOT . "application/admin/" . $posledni.'.php'))
	{
		...
		if (PAGE_ID == 1 && !empty($_GET["url"]) && $_GET["url"] != "root")
		{
			...
			require_once PATH_ADMIN_PRG . '404.php';
			exit;
		}
		require_once PATH_ADMIN_PRG . 'clanky.php';
	}
...
} else
{
		...
		require_once PATH_ADMIN_PRG . 'main.php';
