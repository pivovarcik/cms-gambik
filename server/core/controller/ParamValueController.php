...
class ParamValueController extends G_Controller_Action
{
	...
	public function createAction()
	{
		if($this->getRequest->isPost() && false !== $this->getRequest->getPost('add_param', false))
		{
			...
			if ($form->isValid($this->getRequest->getPost()))
			{
				...
				$param_id = $this->getRequest->getPost('param_id', 0);
				$value = $this->getRequest->getPost('value', '');
				$_productParamValue = new models_ProductParamValue();
				$_productParamValue->setData(array(
									'id_param' => $param_id,
									'string' => $value,
				));
				$id_param_value = $_productParamValue->insert();

				$_productProductParam = new models_ProductParam();
				$_productProductParam->setData(array(
									'id_product' => $product_id,
									'id_param_value' => $id_param_value,
				));

				//$_productProductParam->insert();

				if($_productProductParam->insert())
				{
