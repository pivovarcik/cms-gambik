...
class CatalogFiremController extends CatalogController
{
	public $photos = array();
	public $total = 0;
	...
	function __construct($TCatalog = "CatalogFirem", $TCatalogVersion = "CatalogVersion")
	{
	...
	public function setPageVersionData($postdata, $page_id, $version)
	{
		...
		foreach ($languageList as $key => $val){
			...
			$name = 'registrace';
			if (array_key_exists($name, $postdata)) {
				$versionData[$i][$name] = empty($postdata[$name]) ? null : date("Y-m-d", strtotime($postdata[$name]));
			}
			$name = 'expirace';
			if (array_key_exists($name, $postdata)) {
				$versionData[$i][$name] = empty($postdata[$name]) ? null : date("Y-m-d", strtotime($postdata[$name]));
	...
	public function saveAction()
	{
			...
			)
		{
			...
			// načtu Objekt formu
			$form = $this->formLoad('CatalogFiremEdit');
