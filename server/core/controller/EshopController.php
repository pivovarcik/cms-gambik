...
class EshopController extends G_Controller_Action
{
	...
	public function getSettings()
	{
    	...
    	public function saveRazitkoOrderAjaxAction(){
	...
	//	if($this->getRequest->isPost() && false !== $this->getRequest->getPost('upd-nastaveni', false))
		{
			...
			$fotoDetail = $FotoController->getFoto($id);
     ...
     // print_r($fotoDetail);
		...
		//		$data["value"] = PATH_IMG . $fotoDetail->file;
				$data["value"] = $fotoDetail->dir . $fotoDetail->file;
      ...
      $form->setResultSuccess('Nastavení bylo uloženo.');
    	...
    	public function saveOPOrderAjaxAction(){
		if($this->getRequest->isPost())
	//	if($this->getRequest->isPost() && false !== $this->getRequest->getPost('upd-nastaveni', false))
		{
			...
			$upload_form_name = "file";
			if (isset($_FILES[$upload_form_name])) {
				$FotoController = new FilesController($upload_form_name);
				if ($id = $FotoController->create()) {

           $id = $FotoController->getFileId();
					$key = "OP_PDF_FILE";
					$data = array();
					$data["value"] = $id;
			...
			$fotoDetail = $FotoController->getDetailById($id);
			//$imageController = new ImageController();
	...
	public function saveLogoOrderAjaxAction(){
		if($this->getRequest->isPost())
	//	if($this->getRequest->isPost() && false !== $this->getRequest->getPost('upd-nastaveni', false))
		{
