...
class UserController extends PageController
{
	...
	public function changePasswordProfilAction()
	{
		...
		{
			$form = $this->formLoad('AdminUserPwdChangeEdit');
  ...
  public function removeUserLoginAction()
  {
 		if($this->getRequest->isPost() && false !== $this->getRequest->getPost('user_login_remove', false))
		{
      
      $tokenCookies =  $this->getRequest->getPost('user_login_remove', false);
	...
	public function logInAction()
	{
		...
		if($this->getRequest->isPost() && false !== $this->getRequest->getPost('login', false))
		{
			...
			if ($form->isValid($this->getRequest->getPost()))
			{
				...
				$remember = $this->getRequest->getPost("remember",false);


				$pernament = ($remember) ? true : false;

//print_r($_POST);
				$GAuth = G_Authentification::instance();

				if (!$GAuth->logIn($username, $password, $pernament)) {
	...
	public function logInByEmailAction()
	{
		...
		if($this->getRequest->isPost() && false !== $this->getRequest->getPost('login', false))
		{
			...
			if ($form->isValid($this->getRequest->getPost()))
			{
				...
				$password = $formdata["pwd"];
				$pernament = ($formdata["remember"] == 1) ? true : false;

				$GAuth = G_Authentification::instance();

				if (!$GAuth->logInByEmail($email, $password, $pernament)) {
	...
	public function lostPasswordAction()
	{
        ...
        if (!empty($recaptcha_secret)){
  				...
  				{
  					$form->setResultError($translator->prelozitFrazy("captcha_invalid"));
  					return false;
  				}        
        }
	...
	public function userPanelRender()
	{
		if (LOGIN_STATUS=="ON" && (USER_ROLE_ID == "1" || USER_ROLE_ID == "2"))
		{
			...
			return '<div style="color: rgb(255, 255, 255); display: block; position: absolute; text-align: center; z-index: 99999; font-size: 11px; font-family: arial; border-bottom: 0px none; top: 0px; border-radius: 0px 0px 5px 5px; right: 0px; background-color: rgb(4, 41, 61); padding: 8px 6px;"><a style="color:#fff;" onclick="return confirm(\'Opravdu chcete odhlásit?\')" href="' . URL_HOME2 . 'logout.php">Odhlásit se</a> | <a style="color:#fff;" href="' . URL_HOME2 . 'admin/">Přejít do administrace &rarr;</a></div>';
		}
		return '';
	}
	...
	{
		if($this->getRequest->isPost() && "UserEdit" == $this->getRequest->getPost('action', false))
		{
			// načtu Objekt formu
	...
	{
		if($this->getRequest->isPost() && ( false !== $this->getRequest->getPost('edit_user', false) || false !== $this->getRequest->getPost('user_profil_edit', false)))
		{
			// načtu Objekt formu
			$form = $this->formLoad('AdminUserProfilEdit');
		//	$form = $this->formLoad('AdminUserEdit');
			// Provedu validaci formu
			if ($form->isValid($this->getRequest->getPost()))
			{
			...
			//	$user_id = $formdata["id"];
				...
				$token = $this->getRequest->getSession("uidlogin2", "sdsfsf");

				$userDetail = $models->getUserByToken($token);
			//	$userDetail = $models->getUserById($user_id);
				if (!$userDetail)
				{
					$form->setResultError("Uživatel s tímto ID neexistuje!");
              ...
              $upload_form_name = "F_AdminUserProfilEdit_file";

			//	print_r($_FILES[$upload_form_name]);
			//	exit;
			if (isset($_FILES[$upload_form_name]) && is_array($_FILES[$upload_form_name]) && !empty($_FILES[$upload_form_name]["name"])) {
