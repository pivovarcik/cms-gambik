...
*/
	...
	{
		$request = new G_Html();
		if($request->isPost() && false !== $request->getPost('F_DataGridDefinitionEdit_action', false))
		{
			$filterName = $request->getPost('F_DataGridDefinitionEdit_name', '');
			$columnName = $request->getPost('name[]', array());
			$columnHeader = $request->getPost('header[]', array());
			$columnClass = $request->getPost('class[]', array());
			$columnOrder = $request->getPost('order[]', array());
			$id = $request->getPost('F_DataGridDefinitionEdit_id', 0);
	...
	//	$res = '<form method="post" class="datagrid_filter_definition standard_form" style="display:none;">';
		$res = '<div  class="datagrid_filter_definition standard_form" style="display:none;">';
...
//		print_r($list);
	...
	//	$result .= '<form method="get" class="form-search form-inline">';
		$result .= '<div class="form-search form-inline">';
		$request = new G_Html();
		...
		$elem->setAttribs(array(
						"id" => "LimitFilter",
						"class" => "form-control input-xs LimitFilter"));
		...
		$result .='<label for="rg-from">Zobrazit: </label>';
		$result .= "" . $elem->render() . " na stránku | Celkem nalezeno " . $this->total . " záznamů | ";
		$result .='</div>';
		...
		$elem->setAttribs('class','form-control input-xs search-text');
	...
	public function ajaxTable()
	{
		...
		$res .= '<div id="'.$this->datagridId.'" style="position:relative"></div>';
	...
	//	$res .= 'loadGrid("'.$this->modelName.'");';
		...
		$res .= 'var dataGrid'.$this->modelName.' = new DataGridProvider(args);';
	...
	//	print_r($paramsArray);
      ...
      //print_r($val);
		...
		$res .= 'dataGrid'.$this->modelName.'.setParams(params);';


		if ($this->modalForm) {
			$res .= 'dataGrid'.$this->modelName.'.setModal(true);';
		}


		$res .= 'dataGrid'.$this->modelName.'.loadData();';
	//	$res .= '});';
...
$res .= '$(document).ready(function(){';
	...
	//	$res .= 'console.log("klik");';
		...
		$res .= 'dataGrid'.$this->modelName.'.modalFormCreate($(this).attr("data-url"));';
...
/*
	...
	//	$this->columnsName = $column;
		...
		$result .= $this->actions();
//overflow-x: scroll;
		// class="standard_form" method="get"
		$result .='<div id="filterDefinition2" style=" border: 0 none;">';
...
// celý název už vrací samotný formulář
	/*	if (isset($this->model->formNameEdit)) {
			$res .= 'name = "F_'.$this->model->formNameEdit.'_" + name;';
		}*/
	// prefix
