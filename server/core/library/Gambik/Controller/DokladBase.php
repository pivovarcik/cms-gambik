...
class DokladBase extends ADokladBase {
	...
	public function copyDoklad($doklad_id)
	{
		...
		$doklad->id = null;
		$doklad->code = null;
		$doklad->order_id = null;
		$doklad->order_code = null;
		$doklad->order_date = null;
		$doklad->amount_paid = 0;
		$doklad->pay_account = null;
		$doklad->pay_date = null;
    $doklad->download_hash = null;
		$doklad->faktura_date = date("Ymd");
	...
	{
    // tady příjde pole, proto chyba, nemůže se přetypovat na objekt!!
	//	$doklad->ip_address = $_SERVER["REMOTE_ADDR"];
	//	$doklad->download_hash = strtolower(substr(md5(rand()),0,50)); // Vytvořím unikátní ID
