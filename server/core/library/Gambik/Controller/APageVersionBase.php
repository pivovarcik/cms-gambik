...
abstract class APageVersionBase implements IPageVersion {
	...
	private function setPageDataEntity($postdata, $originalData = null)
	{
		...
		if ( $originalData != null && ($originalData->id)) {
	...
	private function setPageVersionDataEntity($postdata, $pageVersionList, $version, $languageList = array())
	{
		...
		foreach ($languageList as $key => $val){
			...
			$PageVersion->$name = $this->checkDuplicityUrl($url, $val->code, $PageVersion->page_id, $PageVersion->category_id);
	...
	protected function setPageVersionData($postdata, $page_id = null, $version = 0, $languageList = array())
	{
		...
		foreach ($languageList as $key => $val){
			...
			$versionData[$i][$name] = $this->checkDuplicityUrl($url, $val->code, $page_id, $versionData[$i]["category_id"]);
			$i++;
		}
		return $versionData;
	}

	protected function checkDuplicityUrl($url, $lang, $page_id = null, $category_id = null)
	{
		/*if ($url == "root") {
			return $url;
		}*/
	...
	private function getTagId($tag, $lang_id){
		...
		self::$model->insertRecords(T_TAGS,array("lang_id" => $lang_id, "name" => $tag, "url" => strToUrl($tag)));
	//	print self::$model->getLastQuery();
	...
	protected function deleteData($pageSaveData, $form = false)
	{
		...
		$page_id = (int) $pageSaveData->getId();
