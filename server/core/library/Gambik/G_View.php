...
';
	...
	public function render($Obj)
	{
		...
		switch($this->class_name)
		{
			...
			case "G_Form_Element_File":
				...
				array_push($this->render_attrib,"value");
      //  print_r($this->render_attrib);
			//	unset($this->render_attrib["class"]);
			//	$this->render_attrib["class"] = "";
				break;
		...
		$this->_getAttrib($Obj);
         ...
         if ($this->class_name == "G_Form_Element_File")
     {
        
         
         $Obj->Attributes["class"] = str_replace("form-control","",$Obj->Attributes["class"]);
         $Obj->style["element_class"] = str_replace("form-control","",$Obj->style["element_class"]);
        // print_r($Obj);
     }
    ...
    if (isset($Obj->Attributes["after_icon"]) && !empty($Obj->Attributes["after_icon"])) {
        $result = '<div class="input-group">' . $result;
        
      $result .= '<span tabindex="100" class="add-on input-group-addon" style="cursor: pointer;">';
         $result .= $Obj->Attributes["after_icon"];
      $result .= '</span></div>';
    }
