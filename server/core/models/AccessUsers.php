 ...
 * @copyright 2012
 */
class models_AccessUsers extends G_Service {

	function __construct()
	{
		parent::__construct(T_USERS_ACCESS_ASSOC);
	}
	...
	public function getAssociationList($params=array())
	{
		$this->clearWhere();
		...
		if(isset($params['page_id']) && is_int($params['page_id']))
		{
			$pageId = " and ua.page_id=" . $params['page_id'];
		}
		$pageType = "";
		if(isset($params['page_type']) && !empty($params['page_type']))
		{
			$pageType = " and ua.page_type='" . $params['page_type'] . "'";
		}
		...
		{
			$this->addWhere(" ua.user_id=" . $params['user_id']);
		//	$this->addWhere(" u.id=" . $params['user_id']);
		}

		$this->setSelect("u.nick,u.id,case when ua.user_id is not null then 1 else 0 end selected");
		$this->setFrom(T_USERS . " u left join " .$this->getTableName() . " ua ON (ua.user_id = u.id)" . $pageId . $pageType);
