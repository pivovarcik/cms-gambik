...
class models_Translator extends G_Service{
	...
	//	print  $this->getLastQuery();
		...
		//	$list[$i]->link_edit = '/admin/options/translator?do=edit&id='.$list[$i]->id;
			...
			$command = new EditDataGridCommandModal($url_home . "options/translator?do=edit&id=" . $list[$i]->id);
			array_push($list[$i]->dg_commands, $command);

			$command = new DeleteDataGridCommand($url_home . "options/translator?do=TranslatorDelete&id=" . $list[$i]->id);
			array_push($list[$i]->dg_commands, $command);
		}
		return $list;
