...
class models_Message extends G_Service{
	...
	public function getList(IListArgs $params=null)
	{
		...
		$this->setSelect("case when m.autor_id <> " . USER_ID . " and m.ReadTimeStamp is null then 1 else 0 end as isNewMessage,au.nick as autor_nick,
			case when m.ReadTimeStamp is null then 0 else 1 end as precteno,fad.file as fad_file,fau.file as fau_file,
		...
		$this->setFrom($this->getTableName() . " m
		   left join " . T_USERS . " au on m.autor_id=au.id
			left join " . T_USERS . " ad on m.adresat_id=ad.id
      		   ...
      		   left join " . T_FOTO . " fau on fau.id=au.foto_id
			left join " . T_FOTO . " fad on fad.id=ad.foto_id
		...
		for ($i=0;$i < count($list);$i++)
		{
			...
			if (empty($list[$i]->ReadTimeStamp) && $list[$i]->autor_id <> USER_ID && isset($params->adresat_autor) && isInt($params->adresat_autor)) {
				$this->setReader($list[$i]->id);
			}
