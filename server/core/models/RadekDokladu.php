...
");
	...
	public function getList(IListArgs $params = null)
	{
		...
		case when p.id is null then mv.name else mv2.name end as nazev_mj,
	...
	//	$radek = new self::$entitaRadky();
		...
		LEFT JOIN ". T_DPH ." tax ON (t1.tax_id = tax.id)
		LEFT JOIN " . T_MJ . " AS t3 ON t1.mj_id = t3.id
    LEFT JOIN " . T_MJ_VERSION . " mv ON t3.id=mv.mj_id and mv.lang_id=ifnull(l.id,6) 
    
		LEFT JOIN " . T_MJ . " AS t5 ON pv.hl_mj_id = t5.id
    LEFT JOIN " . T_MJ_VERSION . " mv2 ON t5.id=mv2.mj_id and mv2.lang_id=ifnull(l.id,6) 
