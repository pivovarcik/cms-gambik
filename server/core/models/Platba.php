		...
		if(isset($params->doprava_id) && isInt($params->doprava_id))
		{
			...
			$plus_select =  ",case when doprava.platba_id is null then 0 else 1 end as isAllowed
                	...
                	,case when doprava.platba_id is null then 0 else doprava.price_value end as price_value
                		...
                		,case when doprava.platba_id is null then 0 else doprava.price_mj end as price_mj
                		,case when doprava.platba_id is null then 0 else doprava.price_mj_value end as price_mj_value

                                    		,case when doprava.platba_id is null then 0 else doprava.price_m3 end as price_m3
                		,case when doprava.platba_id is null then 0 else doprava.price_m3_value end as price_m3_value
                    
                                    		,case when doprava.platba_id is null then 0 else doprava.price_kg end as price_kg
                		,case when doprava.platba_id is null then 0 else doprava.price_kg_value end as price_kg_value";
                    
			
			$plus_join = " left join (select platba_id,p.price,p.price_value,
      p.price_mj,p.price_mj_value,
      p.price_m3,p.price_m3_value,
      p.price_kg,p.price_kg_value,
      h.value as dph_value from " . T_SHOP_PLATBY_DOPRAVY . " p
...
/*
	...
	public function getPlatbaDopravyList($params=array())
	{
		...
		$this->setSelect("p.*,d.vypocet_id,v.tax_id,h.value as dph_value,v.name as doprava_name, zpv.name as platba_name");
