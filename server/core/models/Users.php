...
class models_Users extends models_Pages{
	...
	{
		$sql = "SELECT UNIX_TIMESTAMP(t1.naposledy) as casnaposledy,f.file,f.dir,
                 t1.*,
...
/*
		...
		*/
		...
		if(isset($params->like_nick) && !empty($params->like_nick))
		{
			$this->addWhere("t1.nick like '" . $params->like_nick . "%'");
		}
		...
		$this->setSelect("UNIX_TIMESTAMP(t1.naposledy) as casnaposledy,t1.*,t2.title as nazev_role,f.file,f.dir,
    substring(t1.email,LENGTH(SUBSTRING_INDEX(t1.email,'@',1))+2,LENGTH(t1.email)) as email_domena
    ");
