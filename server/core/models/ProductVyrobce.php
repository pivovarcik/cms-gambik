...
class models_ProductVyrobce extends ACiselnikModel{

	function __construct()
	{
		parent::__construct(T_SHOP_PRODUCT_VYROBCE);
	}
	...
	public function get_value_from_parent($parent_value)
	{
		...
		*/
		$this->setSelect("t2.*");
		$this->setFrom(T_SHOP_PRODUCT . " t1 left join " . T_SHOP_PRODUCT_VYROBCE . " t2 on t1.vyrobce_id=t2.id");
	...
	public function getList(IListArgs $params = NULL)
	{
		...
		{
		//	$list[$i]->link_edit = URL_HOME . 'eshop/eshop_vyrobce/ProductVyrobceEdit?id='.$list[$i]->id;
			$list[$i]->link_edit = '/admin/eshop/eshop_vyrobce?do=edit&id='.$list[$i]->id;
		}
		return $list;
	}
}