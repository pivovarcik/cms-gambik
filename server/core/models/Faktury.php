...
class models_Faktury extends models_Pages{
	...
	public function getList(IListArgs $params = null)
	{
      ...
      or t1.id in (select doklad_id from " . T_FAKTURA_RADEK . " where product_code like '%" . $params->fulltext . "%' or product_name like '%" . $params->fulltext . "%')
      ) ");
		}
		...
		if(isset($params->code) && !empty($params->code))
		{
			$this->addWhere("t1.code='" .  $params->code . "'");
 			...
 			if (isset($params->neuhrazene) && $params->neuhrazene === true) {
        $this->addWhere("t1.cost_total - ifnull(t1.amount_paid,0) > 0");
      }   
		...
		$this->setSelect("t1.*,dv.name as nazev_dopravy,pv.name as nazev_platby,tf.name as typ_faktury_name,(select product_name from " . T_FAKTURA_RADEK . " where doklad_id=t1.id LIMIT 1) as predmet");
