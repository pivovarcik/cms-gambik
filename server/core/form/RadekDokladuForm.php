...
class RadekDokladuForm extends RadekForm
{
    ...
    $eshopSettings = G_EshopSetting::instance();
		...
		for ($i=0;$i < count($radek);$i++)
		{
			...
			$elem->setAttribs('class',$name . " input-sm");
			...
			$elem2->setAttribs('class',$name . " input-sm");
      ...
      if ($radek[$i]->product_id > 0){
          $this->radky[$i]->product_code .= '<a href="/admin/sortiment/edit_product?id=' . $radek[$i]->product_id . '" target="_blank">' . $value . '</a>';
      } 
