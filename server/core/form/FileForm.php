...
class FileForm extends G_Form
{
	...
	//	$elem->setAttribs('style','width:300px;font-weight:bold;');
		...
		$elem->setAttribs('label','Název souboru:');
		$this->addElement($elem);

    $name = "filename_original";
		$elem = new G_Form_Element_Text($name);
		$elem->setAttribs(array("id"=>$name,"readonly"=>true));
	//	$elem->setAttribs('style','width:300px;font-weight:bold;');
		$value = $this->getPost($name, $page->$name);
		$elem->setAttribs('value',$value);
		$elem->setAttribs('label','Originální název:');
		$this->addElement($elem);
	...
	//	$elem->setAttribs('style','width:300px;font-weight:bold;');
	//	$value = $this->getPost($name, $page->$name);
		$elem->setAttribs('value',URL_IMG . $page->dir . $page->file);
		$elem->setAttribs('label','Url souboru:');
		$this->addElement($elem);
